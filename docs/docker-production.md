...
## Deployed to Azure Container Apps (Block 4.13 → Block 4.23)
...
Block 4.13 introduced the first `linux/amd64` image (`block-4.13-amd64`). Block 4.23 published
`block-4.23-amd64` with the Azure OpenAI `temperature` fix for `gpt-5-mini`. The earlier
`block-4.9` build ran on Apple Silicon without `--platform`, producing a `linux/arm64` image
that Azure Container Apps rejects.
...
See [`infra/terraform/azure/README.md`](../infra/terraform/azure/README.md#block-423--azure-openai-runtime-verification)
for the full Block 4.23 runtime details.
